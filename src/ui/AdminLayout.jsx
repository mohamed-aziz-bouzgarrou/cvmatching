...
import { RiDashboardLine, RiBriefcaseLine, RiBookOpenLine, RiMenuLine, RiCloseLine } from 'react-icons/ri'

const AdminLayout = () => {
  const [open, setOpen] = useState(false)
  const location = useLocation()
  const nav = [
    { to: '/admin/dashboard', label: 'Tableau de bord', icon: <RiDashboardLine /> },
    { to: '/admin/offres', label: "Offres d'emploi", icon: <RiBriefcaseLine /> },
    { to: '/admin/formations', label: 'Formations', icon: <RiBookOpenLine /> },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="flex h-14 items-center justify-between border-b bg-white px-4">
        ...
        <Link to="/candidate/dashboard" className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">Aller au candidat</Link>
      </div>
